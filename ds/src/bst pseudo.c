...
static bst_iter_t NexNodePostOrder(bst_iter_t node)
	return NULL if the tree is empty 

	if there l_son 
		return NexNodePostOrder(l_son)
	else if there r_son
		return NexNodePostOrder(r_son)
	else 
		if node == tree->root
			free(node->parent)/free dummy
			free(node)
			return NULL;
		else
			temp = node->parent
			...
			retur temp
	destroy left
	destroy right  --if is dummy && node->p != NULL: dummy->parent = node->parent, node->parent->right = dummy
	free
	
void BSTDestroy(bst_t *tree);
	
	runner = NexNodePostOrder(tree->root)
	prev_runner = runner
	
	while(NULL != runner)
		runner = NexNodePostOrder(node)
		free(prev_runner);
		prev_runner = runner;
	
	call NodeDestroy(tree->root)
...
if tree empty
	new->r = tree->root /dummy
	tree->root = new

else
	runner = tree->root
	if cmp_func(runner->d, *data) < 0
		 runnr
	while
...
if empty:
	...
	*opt':
	if data is smaller then begin
		insert left to begin
	if data is bigger then end->parent
		insert right to end->parent & update end(dummy)
 ...
 * Return value: A non-valid iterator marking the end o f the tree.
