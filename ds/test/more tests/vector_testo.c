...
#include "vector.h"             /* vector_t and declarations of all its functions */
...
void TestVector(void)
{
	...
	static int push_pop_cumsum[30] = {0};
	int i = 0;
	...
	for (i = 0; i < push_pop_arr_len; ++i)
	{
		...
		if (1 == push_pop_arr[i])
		{
			status = VectorPushBack(vector , (void *) &element);
			if (status != 0)
			{
				failure = 1;
